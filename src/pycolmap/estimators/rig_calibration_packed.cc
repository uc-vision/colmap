...
using FrameIndexArray = py::array_t<uint8_t, py::array::c_style>;
...
std::unique_ptr<CeresRigCalibrator> CreateCeresRigCalibratorPacked(
    ...
    const DoubleArray& frame0_to_frame2_distances,
    ...
    const FrameIndexArray& frame_indices,
  ...
  THROW_CHECK_EQ(frame0_to_frame2_distances.ndim(), 1);
  THROW_CHECK_EQ(frame0_to_frame2_distances.shape(0), num_groups);
  ...
  const auto frame0_to_frame2_distances_view =
      frame0_to_frame2_distances.unchecked<1>();
  ...
  for (ssize_t group_idx = 0; group_idx < num_groups; ++group_idx) {
    ...
    for (size_t frame_idx = 0; frame_idx < 3; ++frame_idx) {
      ...
      group.rigs_from_group[frame_idx] = Rigid3d::FromMatrix(matrix);
    }
    group.frame0_to_frame2_distance.distance =
        frame0_to_frame2_distances_view(group_idx);
    group.frame0_to_frame2_distance.stddev = distance_stddev;
